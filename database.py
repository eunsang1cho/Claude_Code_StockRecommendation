...
_DEFAULT_CONFIGS: dict[str, dict] = {
    "골삼이": {
        "window": 25,          # 최근 N일 이내 대양봉 탐색 기간 (완화 20→25)
        "big_pct": 0.12,       # 대양봉 최소 등락률 (완화 0.15→0.12)
        "vol_mult": 1.8,       # 대양봉 거래량 배수 (20MA 대비, 완화 2.5→1.8)
        "price_tol": 0.07,     # 대양봉 시가 근접 허용 오차 ±7% (완화 3%→7%)
        "ma_tol": 0.07,        # 20MA 근접 허용 오차 ±7% (완화 3%→7%)
        "vol_dec": 0.65,       # 대양봉 이후 거래량 감소 비율 (완화 0.4→0.65)
    ...
    "골든샘플": {
        "window": 16,          # 최근 N일 이내 대양봉 탐색 기간 (완화 12→16)
        "big_pct": 0.12,       # 대양봉 최소 등락률 (완화 0.15→0.12)
        "vol_mult": 1.8,       # 대양봉 거래량 배수 (완화 2.5→1.8)
        "vol_dried": 0.45,     # 거래량 고갈 기준 (완화 0.15→0.45)
        "price_hold": 0.85,    # 대양봉 이후 종가 유지 기준 (완화 0.93→0.85)
        "min_after": 5,        # 대양봉 이후 최소 경과 일수 (완화 7→5)
        "conf_base": 80,       # 기본 신뢰도
    ...
    "레드삼각": {
        ...
        "box_spread": 0.22,    # 박스권 고저 편차 허용 기준 (완화 0.10→0.22)
        "break_start": 60,     # 돌파 구간 시작 (N일 전)
        "break_end": 20,       # 돌파 구간 끝 (N일 전)
        "min_big": 2,          # 돌파 구간 최소 대양봉 수 (완화 3→2)
        "ma_tol": 0.15,        # 60MA 근접 허용 오차 ±15% (완화 3%→15%)
        "box_top_pct": 0.85,   # 박스권 상단 대비 현재가 최소 비율 (완화 0.95→0.85)
    ...
    "MA압축지지": {
        ...
        "big_pct": 0.07,              # 장대양봉 최소 등락률 (완화 0.10→0.07)
        "body_ratio": 0.60,           # 장대양봉 몸통 비율 기준 (60% 이상)
        "vol_mult": 1.8,              # 장대양봉 거래량 배수 (완화 4.0→1.8배)
        "ma20_approach_days_min": 3,  # MA20 접근 확인 최소 경과 일수
        "ma20_approach_days_max": 25, # MA20 접근 확인 최대 경과 일수 (30→25)
        "ma20_near_bottom_tol": 0.06, # MA20 vs 장대 저가 근접 허용 오차 ±6% (완화 2%→6%)
        "ma20_slope_min": 0.0,        # MA20 최소 기울기 (완화 0.002→0.0)
        "atr_ratio_max": 0.08,        # ATR / 현재가 최대 비율 (완화 1.2%→8%)
        "vol_shrink_ratio": 0.8,      # 현재 거래량 ≤ 장대 거래량 × N (완화 0.4→0.8)
        "box_days_min": 5,            # 박스권 최소 확인 일수
        "box_days_max": 20,           # 박스권 최대 확인 일수 (완화 25→20)
        "box_range_pct": 0.20,        # 박스권 허용 등락 범위 (완화 6%→20%)
        "ma20_ma60_conv_tol": 0.07,   # |MA20 - MA60| / 가격 최대 허용치 (완화 2.5%→7%)
    ...
    "텐배거": {
        # ── STEP 1: 시장 리더 ──────────────────────────────────────
        "rs_6m_min":          0.10,   # 6개월 절대 수익률 최소 (완화 0.25→0.10)
        "rs_3m_min":          0.05,   # 3개월 절대 수익률 최소 (완화 0.12→0.05)
        # ── STEP 2: VCP 베이스 ─────────────────────────────────────
        "big_move_pct":       0.40,   # 120일 저점 대비 최소 상승폭 (35%→40%)
        "big_move_lookback":  120,    # 저점 탐색 기간 (거래일)
        "base_days_min":      7,      # 베이스 최소 기간 (완화 10→7)
        "base_days_max":      30,     # 베이스 최대 기간 (35→30)
        "base_range_pct":     0.20,   # 베이스 허용 등락폭 (완화 0.10→0.20)
        "atr_contract":       1.0,    # ATR20_현재 / ATR20_이전 최대 비율 (완화 0.70→1.0)
        "vol_contract":       1.0,    # vol10 / vol50 최대 비율 (완화 0.60→1.0)
        # ── STEP 3: 위치 ───────────────────────────────────────────
        "high52w_ratio":      0.80,   # 현재가 ≥ 52주 고점 × N (완화 0.92→0.80)
        "high60d_ratio":      0.83,   # 현재가 ≥ 60일 고점 × N (완화 0.93→0.83)
